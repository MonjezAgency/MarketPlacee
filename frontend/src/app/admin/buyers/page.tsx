...
export default function AdminBuyersPage() {
    ...
    const loadBuyers = async () => {
        try {
            // Diagnostic counts in parallel — populates the orange banner
            // at the top of the page so the operator can see at a glance
            // "yes, there are N customers in the DB at status X" even
            // when the table below shows zero (e.g. due to a stale tab
            // filter or a deleted JWT).
            const [res, cntRes] = await Promise.all([
                apiFetch('/users?role=CUSTOMER&limit=100', { cache: 'no-store' }),
            ...
            if (res.ok) {
                ...
                console.error('[BUYERS] /users returned', res.status, await res.text().catch(() => ''));
            }
            if (cntRes && cntRes.ok) {
    ...
    const handleBulkAction = async (action: 'approve' | 'block' | 'delete') => {
        ...
        if (!window.confirm(`Are you sure you want to ${action} ${selectedIds.length} buyers?`)) return;

        const tid = toast.loading(`${action.charAt(0).toUpperCase() + action.slice(1)}ing buyers...`);
        ...
        try {
            ...
            if (res.ok) {
                const result = await res.json();
                toast.success(result.message || `Successfully ${action}d buyers`, { id: tid });
                setSelectedIds([]);
                loadBuyers();
            } else {
    ...
    const toggleSelectBuyer = (e: React.MouseEvent, id: string) => {
        e.stopPropagation();
        setSelectedIds(prev => 
    ...
    return (
        <div className="flex flex-col h-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {/* Diagnostic banner — shows the actual DB-level user counts
                so the operator can confirm a registration landed in the
                database even when a tab filter or stale JWT hides it
                                ...
                                <thead className="bg-slate-50/50 border-b border-slate-100">
                                    <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                        <th className="px-6 py-4 w-10">
                                            <input 
                                                type="checkbox" 
                                                checked={selectedIds.length > 0 && selectedIds.length === filteredBuyers.length}
                                    ...
                                    {loading ? (
                                        ...
                                        <tr 
                                            ...
                                            className={cn(
                                                ...
                                                (selectedBuyer?.id === buyer.id || selectedIds.includes(buyer.id)) ? "bg-teal-50/50" : ""
                                            )}
                                        >
                                            <td className="px-6 py-4" onClick={(e) => toggleSelectBuyer(e, buyer.id)}>
                                                <input 
                                                    type="checkbox" 
                    ...
                    >
                        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 shadow-2xl flex items-center justify-between gap-6 text-white ring-1 ring-white/10">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-sm font-bold">Buyers Selected</p>
