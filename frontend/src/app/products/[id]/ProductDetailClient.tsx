                    ...
                    <div className="lg:col-span-5 flex flex-col">
                        <div className="flex flex-col gap-6">
                            ...
                            {(() => {
                                ...
                                const palletsPerShipment = product.palletsPerShipment || 0;

                                let totalPieces = quantity;
                                let breakdown = '';
                                if (unit === 'case' && piecesPerCase > 0) {
                                    totalPieces = quantity * piecesPerCase;
                                    breakdown = `${quantity} × ${piecesPerCase} = ${totalPieces.toLocaleString()} pcs`;
                                    if (casesPerPallet > 0) {
                                        breakdown += ` (${casesPerPallet} cases / pallet)`;
                                    }
                                } else if (unit === 'pallet' && piecesPerPallet > 0) {
                                    totalPieces = quantity * piecesPerPallet;
                                    if (piecesPerCase > 0 && casesPerPallet > 0) {
                                        breakdown = `${quantity} × ${casesPerPallet} × ${piecesPerCase} = ${totalPieces.toLocaleString()} pcs`;
                                    } else {
                                        breakdown = `${quantity} pallet${quantity > 1 ? 's' : ''} × ${piecesPerPallet.toLocaleString()} = ${totalPieces.toLocaleString()} pcs`;
                                    }
                                } else if (unit === 'shipment' && palletsPerShipment > 0 && piecesPerPallet > 0) {
                                    totalPieces = quantity * palletsPerShipment * piecesPerPallet;
                                    breakdown = `${quantity} × ${palletsPerShipment} × ${piecesPerPallet.toLocaleString()} = ${totalPieces.toLocaleString()} pcs`;
                                } else {
                                    breakdown = `${quantity} ${unit}${quantity > 1 ? 's' : ''}`;
                                }
                                const totalPrice = quantity * product.price;

                                return (
                                    <div className="pt-6 mt-2 border-t border-[#E5E7EB]">
                                        <div className="bg-gradient-to-br from-[#F0FDFA] to-[#ECFEFF] border border-[#14B8A6]/20 rounded-2xl p-5 space-y-4">
                                            <div className="flex items-center justify-between">
                                                <p className="text-[10px] font-black text-[#0D9488] uppercase tracking-widest">Logistics Calculator</p>
                                                <span className="px-2 py-0.5 bg-[#14B8A6] text-white text-[9px] font-black uppercase tracking-widest rounded-full">Live</span>
                                            </div>
                                            {(piecesPerCase > 0 || piecesPerPallet > 0) && (
                                                <div className="grid grid-cols-3 gap-2 pb-3 border-b border-[#14B8A6]/15">
                                                    {piecesPerCase > 0 && (
                                                        <div className="text-center">
                                                            <p className="text-[9px] text-[#0D9488] font-bold uppercase tracking-wider">Pieces / Case</p>
                                                            <p className="text-lg font-black text-[#0F172A]">{piecesPerCase}</p>
                                                        </div>
                                                    )}
                                                    {casesPerPallet > 0 && (
                                                        <div className="text-center">
                                                            <p className="text-[9px] text-[#0D9488] font-bold uppercase tracking-wider">Cases / Pallet</p>
                                                            <p className="text-lg font-black text-[#0F172A]">{casesPerPallet}</p>
                                                        </div>
                                                    )}
                                                    {piecesPerPallet > 0 && (
                                                        <div className="text-center">
                                                            <p className="text-[9px] text-[#0D9488] font-bold uppercase tracking-wider">Pieces / Pallet</p>
                                                            <p className="text-lg font-black text-[#0F172A]">{piecesPerPallet.toLocaleString()}</p>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[12px] font-semibold text-[#475569]">Order Quantity</span>
                                                    <span className="text-[12px] font-black text-[#0F172A]">{quantity} {unit}{quantity > 1 ? 's' : ''}</span>
                                                </div>
                                                <div className="flex items-center justify-between">
                                                    <span className="text-[12px] font-semibold text-[#475569]">Total Pieces</span>
                                                    <span className="text-[14px] font-black text-[#14B8A6]">{breakdown}</span>
                                                </div>
                                                <div className="flex items-center justify-between pt-3 border-t border-[#14B8A6]/15">
                                                    <span className="text-[13px] font-black text-[#0F172A] uppercase tracking-wider">Total Price</span>
                                                    <span className="text-2xl font-black text-[#0D9488]">{formatPrice(totalPrice)}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
